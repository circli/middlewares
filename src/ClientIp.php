...
class ClientIp implements MiddlewareInterface
{
	...
	public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface
	{
		...
		return $handler->handle($request->withAttribute(RequestAttributeKeys::IP, $ipAddress));
	}
	...
	protected function determineClientIpAddress($request): string
	{
		...
		if ($checkProxyHeaders && !empty($this->trustedProxies)) {
			if (!\in_array($ipAddress, $this->trustedProxies, true)) {
				$checkProxyHeaders = false;
			}
		}
	 ...
	 * @return boolean
	 */
	protected function isValidIpAddress($ip): bool
	{
